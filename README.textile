...
*Note:*
There is an initializer script that will load the candidate names authorities every time the server is restarted.  This takes a long time, so you probably want to disable for development mode.  You can disable it by setting an environment variable:

  <pre> export NNV_SKIP_CANDIDATES=true </pre>
